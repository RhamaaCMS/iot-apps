...
from .constants import ENVELOPE_VERSION, IOT_MQTT_TOPIC_PREFIX
...
def provision(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        if not all(isinstance(value, str) for value in (token, name, hardware_version)):
            raise ValueError
    ...
    response = JsonResponse(
        {
            ...
            "topic_prefix": f"{IOT_MQTT_TOPIC_PREFIX}/{device.organization.slug}/{device.device_id}",
        },
        status=201,
    )
